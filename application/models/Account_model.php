...
class Account_model extends CI_Model{
	...
	public function getdynamic_sidebar(){
		...
		$parentsidebar = $this->getsidebar_data(array('role_id'=>$role,'status'=>'1','parent'=>'0'),'all');		
		$returnsidebar = array(); 
	...
	public function getsidebar_data($where=array(),$type='all'){
		$table=TP."sidebar";
	...
	public function getall_parentwise_sidebar($allsidebarparentid){
        ...
        if(!empty($allsidebarparentid)){
            foreach($allsidebarparentid as $key=>$oneid){
                $returnarray[$key] =$this->getsidebar_data(array('id'=>$oneid['id'],'status'=>'1'),'single');
                $onesubdata = $this->getsidebar_data(array('parent'=>$oneid['id'],'status'=>'1'),'all');
                if(!empty($onesubdata)){
